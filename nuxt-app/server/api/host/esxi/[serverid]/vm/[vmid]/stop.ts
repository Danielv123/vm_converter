...
export default defineEventHandler(async (event): Promise<string> => {
  ...
  let output = await shell.invoke(
    `Stop-VM -Server ${server.ip} -VM '${event.context.params.vmid.split("%20").join(" ")}' -Confirm:$False | Select-Object -Property Name,Uid,PowerState,NumCpu,CoresPerSocket,MemoryMB,HardwareVersion,PersistendId,GuestId,UsedSpaceGB,ProvisionedSpaceGB,DatastoreIdList,CreateDate,Id | ConvertTo-Json -Compress -Depth 99`,
  );

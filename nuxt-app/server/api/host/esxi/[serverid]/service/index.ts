...
export default defineEventHandler(async (event): Promise<string> => {
  ...
  let output = await shell.invoke(
    `$WarningPreference = 'SilentlyContinue'; Get-VMHostService -Server ${server.ip} | Select-Object -Property Key, Label, Policy, Required, Ruleset, Running, Uninstallable, VMHostId, VMHostUid, Uid, ExtensionData | ConvertTo-Json -Compress -Depth 99`,
  );
